...
public class BetterSchematicsConfig {
    ...
    static {
        ...
        RENDER_DISTANCE = builder
                .comment("Maximum render distance for schematic overlay (blocks)")
                .defineInRange("renderDistance", 256.0, 16.0, 1024.0);
        RENDER_ALPHA = builder
                .comment("Opacity of the schematic overlay (0.0-1.0)")
                .defineInRange("renderAlpha", 0.4, 0.0, 1.0);
        SHOW_MINIMAP = builder
                .comment("Show minimap with build direction")
                .define("showMinimap", true);
        SHOW_MATERIAL_LIST = builder
                .comment("Show material list in HUD")
                .define("showMaterialList", true);
        builder.pop();
    ...
    public static void registerKeys(RegisterKeyMappingsEvent event) {
        openGuiKey = new KeyMapping(
                "key.betterschematics.open_gui",
                GLFW.GLFW_KEY_M,
                "key.categories.betterschematics"
        );
        executePlaceKey = new KeyMapping(
                "key.betterschematics.execute_place",
                GLFW.GLFW_KEY_G,
                "key.categories.betterschematics"
        );
        toggleRenderKey = new KeyMapping(
                "key.betterschematics.toggle_render",
                GLFW.GLFW_KEY_R,
                "key.categories.betterschematics"
        );
        layerUpKey = new KeyMapping(
                "key.betterschematics.layer_up",
                GLFW.GLFW_KEY_UP,
                "key.categories.betterschematics"
        );
        layerDownKey = new KeyMapping(
                "key.betterschematics.layer_down",
                GLFW.GLFW_KEY_DOWN,
                "key.categories.betterschematics"
        );
